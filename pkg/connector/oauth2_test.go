...
// PKCE-mandatory providers flip RequiresPKCE on so the downstream
...
// (a provider definition that carries a non-empty TokenExtraParams) must
// not share the underlying map; mutating one must not be observable on
// the other or in the package-level providerDefinitions.
func TestApplyProviderDefaults_TokenExtraParamsDeepCopy(t *testing.T) {
	t.Parallel()

	const fakeProvider = "TEST_TOKEN_EXTRA_PARAMS_DEEP_COPY"
	previous, hadPrevious := providerDefinitions[fakeProvider]
	providerDefinitions[fakeProvider] = providerDefinition{
	...
	ApplyProviderDefaults(fakeProvider, "https://example.com/cb", c1)
	ApplyProviderDefaults(fakeProvider, "https://example.com/cb", c2)

	require.NotNil(t, c1.TokenExtraParams)
	require.NotNil(t, c2.TokenExtraParams)
	require.Equal(t, "https://api.example.com/v1/", c1.TokenExtraParams["audience"])
	...
	assert.NotContains(t, providerDefinitions[fakeProvider].TokenExtraParams, "sentinel",
