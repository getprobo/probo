...
func (a *Assessor) Assess(
	...
	if a.cfg.Client == nil {
		return nil, fmt.Errorf("cannot assess evidence: client is nil")
	}
	...
	// Extended thinking on Anthropic rejects temperature != 1; skip the
	// temperature override when thinking is enabled and let the provider
	// omit it.
	if a.cfg.Thinking > 0 {
	...
	if err := out.validate(); err != nil {
		return nil, fmt.Errorf("cannot validate evidence assessment: %w", err)
	}

	return &out, nil
}

func (a EvidenceAssessment) validate() error {
	switch a.Confidence {
	...
	if !a.Readable && a.RejectionReason == "" {
		return fmt.Errorf("rejection_reason is required when readable is false")
	}
