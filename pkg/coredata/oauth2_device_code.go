...
`
	...
	rows, err := conn.Query(ctx, q, pgx.StrictNamedArgs{"device_code_hash": hashedValue})
	if err != nil {
		return fmt.Errorf("cannot query oauth2_device_code: %w", err)
	}
...
func (d *OAuth2DeviceCode) Delete(ctx context.Context, conn pg.Conn) error {
	q := `
...
func (d *OAuth2DeviceCode) DeleteExpired(ctx context.Context, conn pg.Conn, now time.Time) (int64, error) {
	q := `
DELETE FROM iam_oauth2_device_codes
WHERE
	expires_at < @now
