...
WHEN NOT MATCHED
	...
	if err != nil {
		return fmt.Errorf("cannot merge processing activity third_parties: %w", err)
	}
...
`
	...
	_, err := conn.Exec(ctx, q, args)
	if err != nil {
		return fmt.Errorf("cannot insert processing activity third_parties: %w", err)
	}

	return nil
}

func (pav ProcessingActivityThirdParties) InsertProcessingActivitySnapshots(
	ctx context.Context,
	conn pg.Tx,
	scope Scoper,
	organizationID gid.GID,
...
INSERT INTO processing_activity_third_parties (tenant_id, processing_activity_id, third_party_id, organization_id, snapshot_id, created_at)
SELECT
	@tenant_id,
	spa.id,
	sv.id,
	...
	@snapshot_id,
	pav.created_at
FROM source_processing_activity_third_parties pav
JOIN snapshot_processing_activities spa ON spa.source_id = pav.processing_activity_id
JOIN snapshot_third_parties sv ON sv.source_id = pav.third_party_id
`
	...
	if err != nil {
		return fmt.Errorf("cannot insert processing activity thirdParty snapshots: %w", err)
	}
