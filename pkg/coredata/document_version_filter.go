...
func (f *DocumentVersionFilter) SQLFragment() string {
	return `(@user_email::text IS NULL OR EXISTS (SELECT 1 FROM document_version_signatures dvs INNER JOIN peoples p ON dvs.signed_by = p.id WHERE dvs.document_version_id = document_versions.id AND p.primary_email_address = @user_email::text))`
}
