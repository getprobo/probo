...
func (vdpa ThirdPartyDataPrivacyAgreements) InsertThirdPartySnapshots(
	ctx context.Context,
	conn pg.Tx,
	scope Scoper,
...
INSERT INTO third_party_data_privacy_agreements (
	tenant_id,
	id,
	snapshot_id,
	source_id,
...
SELECT
	@tenant_id,
	generate_gid(decode_base64_unpadded(@tenant_id), @third_party_data_privacy_agreement_entity_type),
	@snapshot_id,
	vdpa.id,
...
INNER JOIN snapshot_third_parties sv ON sv.source_id = vdpa.third_party_id
WHERE %s AND vdpa.snapshot_id IS NULL
	...
	args := pgx.StrictNamedArgs{
		...
		"third_party_data_privacy_agreement_entity_type": ThirdPartyDataPrivacyAgreementEntityType,
	}
	maps.Copy(args, scope.SQLArguments())

	_, err := conn.Exec(ctx, query, args)
	if err != nil {
		return fmt.Errorf("cannot insert thirdParty data privacy agreement snapshots: %w", err)
	}
