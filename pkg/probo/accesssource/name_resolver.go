...
func (r *hubspotNameResolver) ResolveInstanceName(ctx context.Context) (string, error) {
	...
	var resp struct {
		CompanyName string `json:"companyCurrency"`
...
// figmaNameResolver resolves the Figma organization name.
type figmaNameResolver struct {
	httpClient *http.Client
}

func NewFigmaNameResolver(httpClient *http.Client) NameResolver {
	return &figmaNameResolver{httpClient: httpClient}
}

func (r *figmaNameResolver) ResolveInstanceName(ctx context.Context) (string, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, figmaAPIBaseURL+"/me", nil)
	if err != nil {
		return "", fmt.Errorf("cannot create figma me request: %w", err)
	}

	httpResp, err := r.httpClient.Do(req)
	if err != nil {
		return "", fmt.Errorf("cannot execute figma me request: %w", err)
	}
	defer func() { _ = httpResp.Body.Close() }()

	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		return "", fmt.Errorf("cannot fetch figma me: unexpected status %d", httpResp.StatusCode)
	}

	// The /v1/me endpoint does not directly return an org name.
	// Return empty to fall back to the generic provider label.
	return "", nil
}

// notionNameResolver resolves the Notion workspace name.
...
func (r *notionNameResolver) ResolveInstanceName(ctx context.Context) (string, error) {
	// Notion API does not have a direct workspace name endpoint.
	return "", nil
}
...
// onePasswordNameResolver for 1Password — SCIM bridge has no org name endpoint.
type onePasswordNameResolver struct{}

func NewOnePasswordNameResolver() NameResolver {
	return &onePasswordNameResolver{}
}

func (r *onePasswordNameResolver) ResolveInstanceName(_ context.Context) (string, error) {
	return "", nil
}
