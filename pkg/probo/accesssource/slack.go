...
const (
	slackUsersListEndpoint = "https://slack.com/api/users.list"
)

// SlackDriver fetches workspace users from Slack via OAuth2-authenticated
// REST API requests.
...
func (d *SlackDriver) queryUsers(ctx context.Context, cursor string) (*slackUsersListResponse, error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		return nil, fmt.Errorf("slack users.list request failed with status %d: %s", httpResp.StatusCode, string(bodyBytes))
	}
...
type slackUsersListResponse struct {
	...
	ResponseMetadata slackResponseMetdata `json:"response_metadata"`
}

type slackResponseMetdata struct {
