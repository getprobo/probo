...
func (s *DocumentService) signDocumentVersionInTx(
	...
	if documentVersionSignature.State == coredata.DocumentVersionSignatureStateSigned {
		return nil, fmt.Errorf("document version already signed")
	}
	...
	err := s.svc.pg.WithTx(
		...
		func(conn pg.Conn) error {
			...
			if err := people.LoadByEmailAndOrganizationID(ctx, conn, s.svc.scope, userEmail, documentVersion.OrganizationID); err != nil {
				return fmt.Errorf("cannot find people record for user email %q in organization %q: %w", userEmail, documentVersion.OrganizationID, err)
			}
...
func (s *DocumentService) IsSigned(
	...
	err := s.svc.pg.WithConn(
		...
		func(conn pg.Conn) error {
			var err error
			signed, err = document.IsPublishedVersionSignedByUserEmail(
				ctx,
				conn,
				s.svc.scope,
				documentID,
