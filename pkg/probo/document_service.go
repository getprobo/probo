...
func (s *DocumentService) Get(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) error {
			return document.LoadByID(ctx, conn, predicate, documentID)
		},
	)
	if err != nil {
		return nil, err
...
func (s *DocumentService) GetDefaultApprovers(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return approvers.LoadByDocumentID(ctx, conn, predicate, documentID)
		},
	)
	if err != nil {
	...
	err = s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return profiles.LoadByIDs(ctx, conn, predicate, profileIDs)
		},
	)
	if err != nil {
...
func (s *DocumentService) ListVersionApprovers(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			if err := profiles.LoadByDocumentVersionID(ctx, conn, predicate, documentVersionID, cursor); err != nil {
...
func (s *DocumentService) CountVersionApprovers(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = profiles.CountByDocumentVersionID(ctx, conn, predicate, documentVersionID)
			if err != nil {
...
func (s *DocumentService) GetWithFilter(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			err := document.LoadByIDWithFilter(ctx, conn, predicate, documentID, filter)
			if err != nil {
...
func (s DocumentService) GenerateChangelog(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			if err := draftVersion.LoadLatestVersion(ctx, conn, predicate, documentID); err != nil {
			...
			if document.CurrentPublishedMajor == nil {
				...
				if err := publishedVersion.LoadByDocumentIDAndVersion(ctx, conn, predicate, documentID, *document.CurrentPublishedMajor, *document.CurrentPublishedMinor); err != nil {
	...
	if changelog == nil {
		changelog, err = s.generateChangelog(ctx, predicate, publishedVersion.Content, draftVersion.Content)
		if err != nil {
...
func (s *DocumentService) PublishVersion(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			if err := dv.LoadLatestVersion(ctx, tx, predicate, req.DocumentID); err != nil {
			...
			if req.Minor {
				document, version, err := s.publishMinorVersionInTx(ctx, predicate, tx, req.DocumentID, &req.Changelog, false)
				if err != nil {
			...
			if len(req.ApproverIDs) == 0 {
				document, version, err := s.publishMajorVersionInTx(ctx, predicate, tx, req.DocumentID, &req.Changelog, false)
				if err != nil {
			...
			profiles := &coredata.MembershipProfiles{}
			if err := profiles.LoadByIDs(ctx, tx, predicate, req.ApproverIDs); err != nil {
			...
			document := &coredata.Document{}
			if err := document.LoadByID(ctx, tx, predicate, req.DocumentID); err != nil {
				return fmt.Errorf("cannot load document: %w", err)
			}
			...
			quorum, err := s.svc.DocumentApprovals.RequestApprovalInTx(ctx, predicate, tx, document, dv, req.ApproverIDs, &req.Changelog)
			if err != nil {
				return fmt.Errorf("cannot request approval: %w", err)
			}

			defaultApprovers := &coredata.DocumentDefaultApprovers{}
			if err := defaultApprovers.MergeByDocumentID(ctx, tx, predicate, req.DocumentID, document.OrganizationID, req.ApproverIDs); err != nil {
...
func (s *DocumentService) Create(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			...
			if err := document.Insert(ctx, conn, predicate); err != nil {
				return fmt.Errorf("cannot insert document: %w", err)
			}

			documentVersion.OrganizationID = organization.ID

			if err := documentVersion.Insert(ctx, conn, predicate); err != nil {
			...
			if len(req.DefaultApproverIDs) > 0 {
				...
				if err := approvers.MergeByDocumentID(ctx, conn, predicate, documentID, organization.ID, req.DefaultApproverIDs); err != nil {
...
func (s *DocumentService) SendSigningNotifications(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			if err := signatories.LoadAwaitingSigning(ctx, tx, predicate); err != nil {
			...
			for _, signatory := range signatories {
				...
				if signatory.State != coredata.ProfileStateActive {
					if signatory.Source != coredata.ProfileSourceSCIM {
						...
						if err := invitation.Insert(ctx, tx, coredata.NewPredicateFromObjectID(organizationID)); err != nil {
...
func (s *DocumentService) SignDocumentVersionByIdentity(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			...
			if err := profile.LoadByIdentityIDAndOrganizationID(ctx, conn, predicate, identityID, documentVersion.OrganizationID); err != nil {
				return fmt.Errorf("cannot find profile record for user email in organization %q: %w", documentVersion.OrganizationID, err)
			}

			var signErr error

			documentVersionSignature, signErr = s.signDocumentVersionInTx(ctx, predicate, conn, documentVersionID, profile.ID)
...
func (s *DocumentService) signDocumentVersionInTx(
	...
	if err := documentVersionSignature.LoadByDocumentVersionIDAndSignatory(ctx, conn, predicate, documentVersionID, signatory); err != nil {
	...
	if err := documentVersion.Update(ctx, conn, predicate); err != nil {
		return nil, fmt.Errorf("cannot update document version: %w", err)
	}

	if err := documentVersionSignature.Update(ctx, conn, predicate); err != nil {
...
func (s *DocumentService) updateVersionInTx(
	...
	if err := draftVersion.Update(ctx, tx, predicate); err != nil {
...
func (s *DocumentService) GetVersionSignature(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return documentVersionSignature.LoadByID(ctx, conn, predicate, signatureID)
		},
	)
	if err != nil {
		return nil, err
...
func (s *DocumentService) BulkRequestSignatures(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			profiles := &coredata.MembershipProfiles{}
			if err := profiles.LoadByIDs(ctx, tx, predicate, req.SignatoryIDs); err != nil {
			...
			for _, documentID := range req.DocumentIDs {
				documentVersion := &coredata.DocumentVersion{}
				if err := documentVersion.LoadLatestVersion(ctx, tx, predicate, documentID); err != nil {
				...
				for _, signatoryID := range req.SignatoryIDs {
					signature, err := s.createSignatureRequestInTx(ctx, predicate, tx, documentVersion.ID, signatoryID)
					if err != nil {
...
func (s *DocumentService) createSignatureRequestInTx(
	...
	if err := signatory.LoadByID(ctx, tx, predicate, signatoryID); err != nil {
	...
	err := existingSignature.LoadByDocumentMajorAndSignatory(ctx, tx, predicate, documentVersionID, signatoryID)
	if err == nil {
	...
	documentVersionSignatureID := gid.New(predicate.GetTenantID(), coredata.DocumentVersionSignatureEntityType)
	now := time.Now()
	documentVersionSignature := &coredata.DocumentVersionSignature{
	...
	if err := documentVersionSignature.Insert(ctx, tx, predicate); err != nil {
...
func (s *DocumentService) RequestSignature(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			profile := &coredata.MembershipProfile{}
			if err := profile.LoadByID(ctx, tx, predicate, req.Signatory); err != nil {
			...
			signature, err = s.createSignatureRequestInTx(ctx, predicate, tx, req.DocumentVersionID, req.Signatory)
			if err != nil {
...
func (s *DocumentService) ListSignatures(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return documentVersionSignatures.LoadByDocumentVersionID(ctx, conn, predicate, documentVersionID, cursor, filter)
		},
	)
	if err != nil {
		return nil, err
...
func (s *DocumentService) createDraftInTx(
	...
	draftVersion := &coredata.DocumentVersion{
		ID:             gid.New(predicate.GetTenantID(), coredata.DocumentVersionEntityType),
	...
	if err := draftVersion.Insert(ctx, tx, predicate); err != nil {
...
func (s *DocumentService) deleteDraftInTx(
	...
	if err := draftVersion.Delete(ctx, tx, predicate); err != nil {
...
func (s *DocumentService) SoftDelete(
	...
	return s.svc.pg.WithTx(
		ctx,
		func(ctx context.Context, tx pg.Tx) error {
			if err := s.clearDocumentReferences(ctx, predicate, tx, []gid.GID{documentID}); err != nil {
				return err
			}

			return document.SoftDelete(ctx, tx, predicate)
		},
	)
}
...
func (s *DocumentService) BulkSoftDelete(
	...
	return s.svc.pg.WithTx(
		ctx,
		func(ctx context.Context, tx pg.Tx) error {
			if err := s.clearDocumentReferences(ctx, predicate, tx, documentIDs); err != nil {
				return err
			}

			return documents.BulkSoftDelete(ctx, tx, predicate)
		},
	)
}
...
func (s *DocumentService) BulkArchive(
	...
	return s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			if err := controlDocument.DeleteByDocumentIDs(ctx, tx, predicate, documentIDs); err != nil {
				return fmt.Errorf("cannot delete control mappings: %w", err)
			}

			riskDocument := coredata.RiskDocument{}
			if err := riskDocument.DeleteByDocumentIDs(ctx, tx, predicate, documentIDs); err != nil {
				return fmt.Errorf("cannot delete risk mappings: %w", err)
			}

			measureDocument := coredata.MeasureDocument{}
			if err := measureDocument.DeleteByDocumentIDs(ctx, tx, predicate, documentIDs); err != nil {
			...
			return documents.BulkArchive(ctx, tx, predicate)
		},
	)
}
...
func (s *DocumentService) BulkUnarchive(
	...
	return s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return documents.BulkUnarchive(ctx, conn, predicate)
		},
	)
}
...
func (s *DocumentService) CountVersionsForDocumentID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = documentVersions.CountByDocumentID(ctx, conn, predicate, documentID, filter)
...
func (s *DocumentService) CountSignaturesForVersionID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = documentVersionSignatures.CountByDocumentVersionID(ctx, conn, predicate, documentVersionID, filter)
...
func (s *DocumentService) ListVersions(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			err := documentVersions.LoadByDocumentID(ctx, conn, predicate, documentID, cursor, filter)
			if err != nil {
...
func (s *DocumentService) GetVersion(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return documentVersion.LoadByID(ctx, conn, predicate, documentVersionID)
		},
	)
	if err != nil {
		return nil, err
...
func (s *DocumentService) CountForOrganizationID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = documents.CountByOrganizationID(ctx, conn, predicate, organizationID, filter)
			if err != nil {
...
func (s *DocumentService) CountForControlID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = documents.CountByControlID(ctx, conn, predicate, controlID, filter)
			if err != nil {
				return fmt.Errorf("cannot count documents: %w", err)
			}
...
func (s *DocumentService) ListForControlID(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return documents.LoadByControlID(ctx, conn, predicate, controlID, cursor, filter)
		},
	)
	if err != nil {
		return nil, err
...
func (s *DocumentService) CountForRiskID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = documents.CountByRiskID(ctx, conn, predicate, riskID, filter)
			if err != nil {
				return fmt.Errorf("cannot count documents: %w", err)
			}
...
func (s *DocumentService) ListForRiskID(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			return documents.LoadByRiskID(ctx, conn, predicate, riskID, cursor, filter)
		},
	)
	if err != nil {
		return nil, err
...
func (s *DocumentService) CountForMeasureID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = documents.CountByMeasureID(ctx, conn, predicate, measureID, filter)
			if err != nil {
				return fmt.Errorf("cannot count documents: %w", err)
			}
...
func (s *DocumentService) ListForMeasureID(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			if err := documents.LoadByMeasureID(ctx, conn, predicate, measureID, cursor, filter); err != nil {
...
func (s *DocumentService) Update(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			if err := document.Update(ctx, tx, predicate); err != nil {
				return fmt.Errorf("cannot update document: %w", err)
			}

			// Handle draft version logic for title/content/classification/type changes.
			latestVersion := &coredata.DocumentVersion{}
			if err := latestVersion.LoadLatestVersion(ctx, tx, predicate, req.DocumentID); err != nil {
				return fmt.Errorf("cannot load latest version: %w", err)
			}
			...
			if !hasVersionChanges {
				if req.DefaultApproverIDs != nil {
					defaultApprovers := &coredata.DocumentDefaultApprovers{}
					if err := defaultApprovers.MergeByDocumentID(ctx, tx, predicate, req.DocumentID, document.OrganizationID, *req.DefaultApproverIDs); err != nil {
						return fmt.Errorf("cannot update default approvers: %w", err)
					}
				}
			...
			if latestVersion.Status == coredata.DocumentVersionStatusDraft {
				...
				if err := s.updateVersionInTx(ctx, predicate, tx, latestVersion, req.Content, req.Classification, req.DocumentType, req.Title); err != nil {
					return err
				}
				...
				if document.CurrentPublishedMajor != nil && (latestVersion.Major != 0 || latestVersion.Minor != 1) {
					...
					if latestVersion.Title == publishedVersion.Title &&
						...
						if err := s.deleteDraftInTx(ctx, predicate, tx, latestVersion); err != nil {
							return err
						}
				...
				draftVersion, err := s.createDraftInTx(ctx, predicate, tx, document, latestVersion)
				if err != nil {
					return err
				}

				if err := s.updateVersionInTx(ctx, predicate, tx, draftVersion, req.Content, req.Classification, req.DocumentType, req.Title); err != nil {
					return err
				}
...
func (s *DocumentService) DeleteDraft(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			latestVersion := &coredata.DocumentVersion{}
			if err := latestVersion.LoadLatestVersion(ctx, tx, predicate, documentID); err != nil {
				return fmt.Errorf("cannot load latest version: %w", err)
			}
			...
			return s.deleteDraftInTx(ctx, predicate, tx, latestVersion)
		},
	)
	if err != nil {
		return nil, err
...
func (s *DocumentService) Archive(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			controlDocument := coredata.ControlDocument{}
			if err := controlDocument.DeleteByDocumentIDs(ctx, tx, predicate, []gid.GID{documentID}); err != nil {
				return fmt.Errorf("cannot delete control mappings: %w", err)
			}

			riskDocument := coredata.RiskDocument{}
			if err := riskDocument.DeleteByDocumentIDs(ctx, tx, predicate, []gid.GID{documentID}); err != nil {
				return fmt.Errorf("cannot delete risk mappings: %w", err)
			}

			measureDocument := coredata.MeasureDocument{}
			if err := measureDocument.DeleteByDocumentIDs(ctx, tx, predicate, []gid.GID{documentID}); err != nil {
				return fmt.Errorf("cannot delete measure mappings: %w", err)
			}
...
func (s *DocumentService) CancelSignatureRequest(
	...
	return s.svc.pg.WithTx(
		ctx,
		func(ctx context.Context, tx pg.Tx) error {
			if err := documentVersionSignature.LoadByID(ctx, tx, predicate, documentVersionSignatureID); err != nil {
				return fmt.Errorf("cannot load document version signature: %w", err)
			}

			documentVersion := &coredata.DocumentVersion{}
			if err := documentVersion.LoadByID(ctx, tx, predicate, documentVersionSignature.DocumentVersionID); err != nil {
				return fmt.Errorf("cannot load document version: %w", err)
			}
			...
			if err := documentVersionSignature.Delete(ctx, tx, predicate, documentVersionSignatureID); err != nil {
...
func (s *DocumentService) ExportPDF(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) (err error) {
			data, err = exportDocumentPDF(ctx, s.svc, s.html2pdfConverter, conn, predicate, documentVersionID, options)
			if err != nil {
...
func (s *DocumentService) BuildAndUploadExport(ctx context.Context, predicate coredata.Predicater, exportJobID gid.GID) (*coredata.ExportJob, error) {
	exportJob := &coredata.ExportJob{}
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			if err := firstDocument.LoadByID(ctx, tx, predicate, documentIDs[0]); err != nil {
			...
			err = s.Export(ctx, predicate, documentIDs, tempFile, exportOptions)
			if err != nil {
...
func exportDocumentPDF(
	...
	version := &coredata.DocumentVersion{}
	if err := version.LoadByID(ctx, conn, predicate, documentVersionID); err != nil {
		return nil, fmt.Errorf("cannot load document version: %w", err)
	}
	...
	if version.FileID != nil {
		return exportStoredPDF(ctx, svc, html2pdfConverter, conn, predicate, version, options)
	}
	...
	pdfData, err := generateDocumentPDF(ctx, svc, html2pdfConverter, conn, predicate, version, generateOptions)
	if err != nil {
		return nil, err
	}

	if options.WithSignatures {
		signaturePagePDF, err := generateSignaturePagePDF(ctx, svc, html2pdfConverter, conn, predicate, version)
		if err != nil {
...
func generateSignaturePagePDF(
	...
	if err := signaturesWithPeople.LoadByDocumentVersionIDWithPeople(ctx, conn, predicate, version.ID, 1_000); err != nil {
...
func generateDocumentPDF(
	...
	if err := document.LoadByID(ctx, conn, predicate, version.DocumentID); err != nil {
	...
	if err := lastQuorum.LoadLastByDocumentVersionID(ctx, conn, predicate, version.ID); err != nil {
		...
		if len(approverProfileIDs) > 0 {
			...
			if err := approverProfiles.LoadByIDs(ctx, conn, predicate, approverProfileIDs); err != nil {
				return nil, fmt.Errorf("cannot load approver profiles: %w", err)
			}
	...
	if organization.HorizontalLogoFileID != nil {
		...
		fileErr := fileRecord.LoadByID(ctx, conn, predicate, *organization.HorizontalLogoFileID)
		if fileErr == nil {
	...
	return s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			for i, documentID := range documentIDs {
				...
				documentVersion := &coredata.DocumentVersion{}
				if err := documentVersion.LoadLatestVersion(ctx, conn, predicate, documentID); err != nil {
...
func (s *DocumentService) SendExportEmail(
	...
	return s.svc.pg.WithTx(
		...
		func(ctx context.Context, tx pg.Tx) error {
			...
			downloadURL, err := s.GenerateDocumentExportDownloadURL(ctx, predicate, file)
			if err != nil {
...
func (s *DocumentService) publishMajorVersionInTx(
	...
	document, documentVersion, err := s.loadDraftForPublish(ctx, predicate, tx, documentID, ignoreExisting)
	if err != nil {
		return nil, nil, err
	}
	...
	if document.CurrentPublishedMajor != nil {
		publishedVersion := &coredata.DocumentVersion{}
		if err := publishedVersion.LoadByDocumentIDAndVersion(ctx, tx, predicate, documentID, *document.CurrentPublishedMajor, *document.CurrentPublishedMinor); err != nil {
	...
	if err := s.finalizePublish(ctx, predicate, tx, document, documentVersion, changelog); err != nil {
		return nil, nil, err
	}

	if err := s.cancelPreviousMajorSignatureRequestsInTx(ctx, predicate, tx, documentID, documentVersion.Major); err != nil {
		return nil, nil, err
	}
...
func (s *DocumentService) cancelPreviousMajorSignatureRequestsInTx(
	...
	if err := signatures.DeleteRequestedByDocumentIDBelowMajor(ctx, tx, predicate, documentID, major); err != nil {
