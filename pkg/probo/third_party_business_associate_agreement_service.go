...
func (s ThirdPartyBusinessAssociateAgreementService) GetByThirdPartyID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) error {
			thirdPartyBusinessAssociateAgreement = &coredata.ThirdPartyBusinessAssociateAgreement{}
			if err := thirdPartyBusinessAssociateAgreement.LoadByThirdPartyID(ctx, conn, predicate, thirdPartyID); err != nil {
				return fmt.Errorf("cannot load thirdParty business associate agreement: %w", err)
			}

			file = &coredata.File{}
			if err := file.LoadByID(ctx, conn, predicate, thirdPartyBusinessAssociateAgreement.FileID); err != nil {
...
func (s ThirdPartyBusinessAssociateAgreementService) Upload(
	...
	err = s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			...
			thirdPartyBusinessAssociateAgreementID := gid.New(predicate.GetTenantID(), coredata.ThirdPartyBusinessAssociateAgreementEntityType)
			...
			if err := file.Insert(ctx, conn, predicate); err != nil {
				return fmt.Errorf("cannot insert file: %w", err)
			}

			if err := thirdPartyBusinessAssociateAgreement.Upsert(ctx, conn, predicate); err != nil {
...
func (s ThirdPartyBusinessAssociateAgreementService) Get(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) error {
			thirdPartyBusinessAssociateAgreement = &coredata.ThirdPartyBusinessAssociateAgreement{}
			if err := thirdPartyBusinessAssociateAgreement.LoadByID(ctx, conn, predicate, thirdPartyBusinessAssociateAgreementID); err != nil {
				return fmt.Errorf("cannot load thirdParty business associate agreement: %w", err)
			}
...
func (s ThirdPartyBusinessAssociateAgreementService) Update(
	...
	err := s.svc.pg.WithTx(
		ctx,
		func(ctx context.Context, conn pg.Tx) error {
			if err := existingAgreement.LoadByThirdPartyID(ctx, conn, predicate, thirdPartyID); err != nil {
			...
			if err := existingAgreement.Update(ctx, conn, predicate); err != nil {
				return fmt.Errorf("cannot update thirdParty business associate agreement: %w", err)
			}

			if err := file.LoadByID(ctx, conn, predicate, existingAgreement.FileID); err != nil {
				return fmt.Errorf("cannot load file: %w", err)
			}
...
func (s ThirdPartyBusinessAssociateAgreementService) Delete(
	...
	return s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			...
			if err := thirdPartyBusinessAssociateAgreement.Delete(ctx, conn, predicate); err != nil {
...
func (s ThirdPartyBusinessAssociateAgreementService) DeleteByThirdPartyID(
	...
	return s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			...
			if err := thirdPartyBusinessAssociateAgreement.DeleteByThirdPartyID(ctx, conn, predicate, thirdPartyID); err != nil {
				return fmt.Errorf("cannot delete thirdParty business associate agreement: %w", err)
			}
