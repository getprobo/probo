...
func (s RightsRequestService) Get(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) error {
			if err := request.LoadByID(ctx, conn, predicate, rightsRequestID); err != nil {
...
func (s *RightsRequestService) Create(
	...
	request := &coredata.RightsRequest{
		ID:             gid.New(predicate.GetTenantID(), coredata.RightsRequestEntityType),
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			...
			if err := request.Insert(ctx, conn, predicate); err != nil {
...
func (s *RightsRequestService) Update(
	...
	err := s.svc.pg.WithTx(
		ctx,
		func(ctx context.Context, conn pg.Tx) error {
			if err := request.LoadByID(ctx, conn, predicate, req.ID); err != nil {
				return fmt.Errorf("cannot load rights request: %w", err)
			}
			...
			if err := request.Update(ctx, conn, predicate); err != nil {
...
func (s *RightsRequestService) Delete(
	...
	err := s.svc.pg.WithTx(
		...
		func(ctx context.Context, conn pg.Tx) error {
			...
			if err := request.Delete(ctx, conn, predicate); err != nil {
...
func (s RightsRequestService) CountByOrganizationID(
	...
	err := s.svc.pg.WithConn(
		...
		func(ctx context.Context, conn pg.Querier) (err error) {
			...
			count, err = requests.CountByOrganizationID(ctx, conn, predicate, organizationID)
			if err != nil {
...
func (s RightsRequestService) ListForOrganizationID(
	...
	err := s.svc.pg.WithConn(
		ctx,
		func(ctx context.Context, conn pg.Querier) error {
			err := requests.LoadByOrganizationID(ctx, conn, predicate, organizationID, cursor)
			if err != nil {
