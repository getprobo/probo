...
var (
	...
	templateFuncs = template.FuncMap{
		...
		"calculateActivityPage": func(index, total int) int {
			const firstPageTOCItems = 21
			const otherPageTOCItems = 28

			tocPages := 1
			if total > firstPageTOCItems {
				remainingItems := total - firstPageTOCItems
				additionalPages := (remainingItems + otherPageTOCItems - 1) / otherPageTOCItems
				tocPages = 1 + additionalPages
			}

			return tocPages + index + 1
		},
		"calculateAssessmentPage": func(index, total int) int {
			const firstPageTOCItems = 21
			const otherPageTOCItems = 28
