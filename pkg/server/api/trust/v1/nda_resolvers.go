...
func (r *nonDisclosureAgreementResolver) FileURL(ctx context.Context, obj *types.NonDisclosureAgreement) (string, error) {
	...
	if identity := authn.IdentityFromContext(ctx); identity != nil && r.esign != nil {
		...
		access, err := trustService.TrustCenterAccesses.GetAccess(ctx, predicate, trustCenter.ID, identity.ID)
	...
	fileURL, err := trustService.TrustCenters.GenerateNDAFileURL(ctx, predicate, trustCenter.ID, 15*time.Minute)
	if err != nil {
