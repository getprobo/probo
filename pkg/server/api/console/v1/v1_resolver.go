...
func (r *mutationResolver) UpdateRightsRequest(ctx context.Context, input types.UpdateRightsRequestInput) (*types.UpdateRightsRequestPayload, error) {
	...
	var requestType *coredata.RightsRequestType
	if input.RequestType.IsSet() {
		requestType = input.RequestType.Value()
	}

	var requestState *coredata.RightsRequestState
	if input.RequestState.IsSet() {
		requestState = input.RequestState.Value()
	}
