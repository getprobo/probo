...
func (r *measureResolver) TreatmentPlans(ctx context.Context, obj *types.Measure, first *int, after *page.CursorKey, last *int, before *page.CursorKey, orderBy *types.TreatmentPlanOrderBy) (*types.TreatmentPlanConnection, error) {
	...
	page, err := r.riskManagement.ListTreatmentPlansForMeasureID(ctx, scope, obj.ID, cursor)
	if err != nil {
		r.logger.ErrorCtx(ctx, "cannot list measure treatment plans", log.Error(err))
		return nil, gqlutils.Internal(ctx)
	}

	return types.NewTreatmentPlanConnection(page, r, obj.ID), nil
}
...
type (
	measureResolver           struct{ *Resolver }
	measureConnectionResolver struct{ *Resolver }
)
