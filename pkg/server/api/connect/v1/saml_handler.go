...
func NewSAMLHandler(saml *saml.Service, iam *iam.Service, cookieConfig securecookie.Config, baseURL string) *SAMLHandler {
	base, err := baseurl.Parse(baseURL)
	if err != nil {
		panic(fmt.Errorf("cannot parse base URL: %w", err))
	}

	return &SAMLHandler{saml: saml, iam: iam, cookieConfig: cookieConfig, baseURL: base}
}

func (h *SAMLHandler) MetadataHandler(w http.ResponseWriter, r *http.Request) {
	metadataXML, err := h.saml.GenerateSpMetadata()
	if err != nil {
...
func (h *SAMLHandler) ConsumeHandler(w http.ResponseWriter, r *http.Request) {
	...
	user, membership, err := h.saml.HandleAssertion(ctx, samlResponse, configID)
	if err != nil {
...
func (h *SAMLHandler) LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	url, err := h.saml.InitiateLogin(ctx, samlConfigID)
	if err != nil {
