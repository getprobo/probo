...
type (
	...
	SendPasswordResetInstructionByEmailRequest struct {
		Email string
	}
...
func (req SendPasswordResetInstructionByEmailRequest) Validate() error {
	v := validator.New()
	v.Check(req.Email, "email", validator.Email())
	return v.Error()
}
...
func (s AuthService) SendPasswordResetInstructionByEmail(
	ctx context.Context,
	req *SendPasswordResetInstructionByEmailRequest,
) error {
	if err := req.Validate(); err != nil {
		return fmt.Errorf("invalid request: %w", err)
	}
	...
	token, err := statelesstoken.NewToken(
		...
		PasswordResetData{Email: req.Email},
	)
	if err != nil {
	...
	return s.pg.WithTx(
		...
		func(tx pg.Conn) error {
			user := &coredata.User{}
			if err := user.LoadByEmail(ctx, tx, req.Email); err != nil {
				if err == coredata.ErrResourceNotFound {
					return nil // Don't leak information about non-existent users
				}
...
func (s AuthService) OpenSessionWithPassword(ctx context.Context, email string, password string) (*coredata.User, *coredata.Session, error) {
	v := validator.New()
	v.Check(email, "email", validator.Email())
	v.Check(password, "password", PasswordValidator())

	err := v.Error()
	if err != nil {
		return nil, nil, &ErrInvalidCredentials{err.Error()}
	}
	...
	err = s.pg.WithTx(
		...
		func(conn pg.Conn) error {
			...
			if user == nil {
				s.hp.ComparePasswordAndHash([]byte(password), []byte("qwertyuiop1234567890"))
