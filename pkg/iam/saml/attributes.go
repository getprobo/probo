...
func extractUserAttributes(assertion *saml.Assertion, config *coredata.SAMLConfiguration) (string, string, *coredata.MembershipRole, error) {
	var (
	...
	if len(assertion.AttributeStatements) == 0 {
		...
		return "", "", nil, fmt.Errorf("no attribute statement and no NameID in assertion")
	}

	email, err := extractAttributeValue(assertion, config.AttributeEmail)
	if err != nil {
		if assertion.Subject != nil && assertion.Subject.NameID != nil {
			email = assertion.Subject.NameID.Value
		} else {
			return "", "", nil, fmt.Errorf("cannot extract email: %w", err)
		}
	}
