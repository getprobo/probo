...
export const paginatedStateOfApplicabilityFragment = graphql`
  ...
  @argumentDefinitions(
    ...
    stateOfApplicabilities(
      ...
      orderBy: $order
      filter: $filter
    ) @connection(key: "StateOfApplicabilityGraphPaginatedQuery_stateOfApplicabilities") {
      __id
      edges {
        node {
          id
...
export function useStateOfApplicabilityQuery(
  ...
  const stateOfApplicabilities = pagination.data.stateOfApplicabilities?.edges.map((edge) => edge.node);
  return {
    ...pagination,
    stateOfApplicabilities,
    connectionId: pagination.data.stateOfApplicabilities.__id,
  };
}
...
export const StateOfApplicabilityConnectionKey = "StateOfApplicabilityGraphPaginatedQuery_stateOfApplicabilities";
