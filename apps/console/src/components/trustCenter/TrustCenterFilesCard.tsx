...
import type { TrustCenterFilesCardFragment$key } from "./__generated__/TrustCenterFilesCardFragment.graphql";
...
function FileRow(props: {
  file: TrustCenterFilesCardFragment$key;
  onChangeVisibility: (fileId: string, trustCenterVisibility: "NONE" | "PRIVATE" | "PUBLIC") => void;
  onEdit: (file: { id: string; name: string; category: string }) => void;
  onDelete: (id: string) => void;
  disabled?: boolean;
}) {
  const file = useFragment(trustCenterFileFragment, props.file);
  const { __ } = useTranslate();
