...
export function renderBanner(
  ...
  const renderMainContent = () => {
    ...
    rejectBtn.setAttribute("aria-label", config.reject_all_label);
    ...
    acceptBtn.setAttribute("aria-label", config.accept_all_label);
    ...
    customizeBtn.setAttribute("aria-label", strings.customize);
  ...
  // Focus trap: cycle Tab within the banner.
  overlay.addEventListener("keydown", (e: KeyboardEvent) => {
    if (e.key !== "Tab") return;
    const focusable = Array.from(
      overlay.querySelectorAll<HTMLElement>("button, a, input, [tabindex]"),
    );
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      // Auto-focus the first button after animation.
