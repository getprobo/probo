...
export class ConsentManager {
  ...
  constructor(private readonly managerConfig: ConsentManagerConfig) {
    ...
    this.consentMode = managerConfig.consentMode ?? "opt-in"; // Overridden by server config
  ...
  async init(): Promise<void> {
    ...
    // Use server-side consent mode if provided.
    const serverMode = config.consent_mode;
    if (serverMode === "opt-out") {
      this.consentMode = "opt-out";
    } else if (serverMode === "opt-in") {
      this.consentMode = "opt-in";
    }
    ...
    // If consent exists, is valid, version matches, and not expired — apply silently.
    if (
      stored &&
      stored.version === config.version &&
      !this.isConsentExpired(stored)
    ) {
  ...
  needsConsent(): boolean {
    ...
    if (stored.version !== this.config.version) return true;
    return this.isConsentExpired(stored);
  }
  ...
  private isConsentExpired(stored: import("./types").StoredConsent): boolean {
    if (!this.config || !stored.timestamp) return false;
    const expiryMs = this.config.consent_expiry_days * 24 * 60 * 60 * 1000;
    return Date.now() - stored.timestamp > expiryMs;
  }
