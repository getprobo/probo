...
MCP, and Trust packages sequentially because each package starts probod on the
same fixed ports. Trust uses `-parallel=1` so managed-domain certificates are
provisioned without worker contention.
