...
generic `/probot bind` prompt in the conversation (no token, no DM). The
Slackbot slash command Request URL is `https://<baseUrl>/slack/v1/commands`;
`/probot bind` replies with an ephemeral Block Kit button whose URL contains
...
`probod` starts exactly one provider-neutral `agent-run-worker`. It schedules
both one-shot GraphQL runs and conversational executions from
`agent_runs`/`agent_inputs`, using the agent-profile and execution-adapter
registries. Conversational `agent_runs` appear only after someone `@mentions`
the bot. It also starts a provider-neutral `probot-message-worker` that claims
`bot_messages`, renders via `OutboundMessageCapability`, and hands the intent
to the channel adapter. These workers run even when Slackbot is disabled;
provider-specific ingress and delivery workers only start when their provider
is enabled. When Slackbot is disabled, compliance workflows do not enqueue
bot messages, so a deployment without Slack remains fully functional without
accumulating jobs that have no registered adapter.
...
turns: they do not create `agent_runs` or `agent_inputs`.
...
The global scheduler claims an eligible execution with
`FOR UPDATE SKIP LOCKED` and a random owner token. The provider adapter
...
input's identity. Only `USER` inputs exist. Each conversational claim processes
one USER input so concurrent pings in a shared thread do not share the last
writer's authorization. Assistant final text is never auto-delivered:
conversational channel-visible effects are queued by channel tools with stable
operation keys.
...
an acknowledged no-op. Channel agents start only on `@mention`. The Slack
adapter fetches `conversations.replies` outside the database transaction,
formats a thread snapshot, creates the execution if none exists for that
thread, and enqueues a USER input. Reactions and channel message edits do not
start agents. DMs keep a per-user session. The event worker transitions:
...
Conversational agent runs and their USER inputs:
...
table per transaction. One-shot GraphQL executions are not removed by this
reliability retention worker.
...
4. Coredata migrations through `20260813T103123Z` have been applied.
5. Alerts exist for dead letters, queue age, and `backend=legacy-slack`
   log volume.
